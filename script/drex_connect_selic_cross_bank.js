...
async function clientTransferRtToExternalClient(
  ...
  try {
    const txSendRTApprove = await senderRealTokenizado.approve(
      fromBankInfo.dvpEscrotingAddress,
      amount
    );
    const approvalReceipt = await txSendRTApprove.wait(0);
  ...
  const txDvpEscorting = await DvpEscorting.scheduleBurn(scheduleRequest);
  await txDvpEscorting.wait();
  ...
  console.log(`\nRealDigital address is: ${await realDigital.getAddress()}\n`);
...
// 1. INSTALLING ALL CENTRAL BANK SMART CONTRACTS ON L2
// deployDrex().then();

// 2. TPFt WHOLESALE WORKFLOW
// bankBuyTpftFromOtherBankProcess().then();

// 3. TPFt RETAIL WORKFLOW
// clientBuyTpftFromExternalClientProcess().then();

// 4. INTERNAL BANK CLIENT TRANSFER
// client1TransferClinet3InBank1().then()

// 5. CROSS BANK CLIENT TRANSFER
client1InBank1TransferClient2InBank2().then();
