...
async function task (title, step, total_steps, callback, args = []) {

  console.info(`\n 🔹 Starting ${title}.  Step ${step} of ${total_steps}\n`)
  // await callback.apply(null, args)
  console.info(`\n ✅ Done step ${step} of ${total_steps}\n`)
...
async function runTasks (tasks = []) {
  if (!!tasks.length) {
    ...
    console.log(`The ${steps} steps are: \n`)
    console.log(tasks.map((t, index) => `${index+1}. ${t.title}`).join("\n"))
    console.log("\n")

    for await (const task of tasks) {
      step = increaseStep(step)
      console.log(`🔹 ${step}. Starting: ${task.title}`)
      if (typeof task.call instanceof Promise ? await task.fn.apply(null, task.args) : task.fn.call(null, task.args))
...
async function deployDrex() {
  try {
    ...
    await runTasks([
      {
        title: `Task 1`,
        fn: function(name) { return `[RUNNING TASK: ${name}]` },
        args: []
      },
      {
        title: `Task 2`,
    ...
    let step = 0
    step = increaseStep(step)
    await task('Deploy Central Bank node contracts', step, total_steps, deployOnCentralBankNode);
    step = increaseStep(step)
    await task('Deploy Selic node contracts', step, total_steps, deployOnSelicNode);
    step = increaseStep(step)
    await task('Deploy Bank A node contracts', step, total_steps, deployOnBankNode, [bank1Info]);
    step = increaseStep(step)
    await task('Deploy Bank B node contracts', step, total_steps, deployOnBankNode, [bank2Info]);
    step = increaseStep(step)
    await task(`Authorizing and granting on the ${centralBankInfo.bankName} node`, step, total_steps, authorizeOnCentralBankNode);
    step = increaseStep(step)
    await task(`Authorizing and granting on the ${selicInfo.bankName} node`, step, total_steps, authorizeOnSelicNode);
    step = increaseStep(step)
    await task(`Authorizing and granting Client 1 on the ${bank1Info.bankName} node`, step, total_steps, authorizeOnBankNode, [bank1Info, client1]);
    step = increaseStep(step)
    await task(`Authorizing and granting Client 2 on the ${bank1Info.bankName} node`, step, total_steps, authorizeOnBankNode, [bank2Info, client2]);
    step = increaseStep(step)
    await task(`Authorizing and granting Client 3 on the ${bank1Info.bankName} node`, step, total_steps, authorizeOnBankNode, [bank1Info, client3]);
    step = increaseStep(step)
    await task(`Creating TPFt to the ${selicInfo.bankName} wallet`, step, total_steps, createTpft);
  } catch (error) {
    console.log(error)
  }
}
