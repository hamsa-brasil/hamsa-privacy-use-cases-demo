...
//TODO 冻结功能，钱是显示还是消失？如果消失，到了谁的账户？
//TODO 转账时应该考虑冻结的金额无法转账
contract HamsaToken is ERC20Burnable {
    // Events
    event HamsaTransferMade(
    ...
    function transfer(
        ...
        (uint256 fromBalance, uint256 toBalance) = getFromAndToBalance(
            msg.sender,
            to
        );
    ...
    function unfreezeAccount(
        ...
        frozen[unfreezer] = Frozen(unfreezer, frozen[unfreezer].value - amount);
    }

    function getFromAndToBalance(
        address from,
        address to
